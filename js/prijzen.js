 ...
 * @file js/prijzen.js
 * @version 1.0.0
 * @description Page script for abonnementen/prijzen.html
 *              Loaded AFTER i18nModule.init() resolves to prevent the race condition
 *              where updateContent() runs before the 'prijzen' namespace is available,
 *              causing keys to render as their key name (e.g. 'tabel.title').
 *
 *              Responsibilities:
 *              - Load namespace 'prijzen' for all three languages
 ...
 * @param {string} key       - Full i18n key including namespace, e.g. 'prijzen:donatie.noot'
 */
...
function _translateHTMLBlocks() {
    _translateHTML('donatie-noot', 'prijzen:donatie.noot'); // Donation note: contains <strong> tag
}
...
// Load the 'prijzen' namespace for the active language (and fallback language).
...
// to re-translate all [data-i18n] elements — this time with 'prijzen' available.
i18nModule.loadNamespace('prijzen').then(function() {
    ...
    // then inject topbar.js to guarantee #top-auth is in the DOM before topbar.js runs
    fetch('/MyFamTreeCollab/Layout/TopBar.html')
        ...
        .then(function(data) {
            ...
            document.body.appendChild(s);                                                      // Append to body — browser executes immediately
        });
    ...
    SiteAnalytics.trackPage('prijzen'); // Register visit in Supabase page_visits table
});
