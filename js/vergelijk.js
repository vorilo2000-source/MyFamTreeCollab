/**
 * @file js/vergelijk.js
 * @version 1.0.0
 * @description Page script for abonnementen/vergelijk.html.
 *              Loaded AFTER i18nModule.init() resolves to prevent the race condition
 *              where updateContent() runs before the 'vergelijk' namespace is available,
 *              causing keys to render as their key name (e.g. 'tabel.title').
 *
 *              Responsibilities:
 *              - Load namespace 'vergelijk'
 ...
 * Used for elements that contain HTML tags (<strong>, <a>) which would be
 * stripped by textContent (used by data-i18n / updateContent).
 *
 * @param {string} elementId - ID of the target DOM element
 * @param {string} key       - Full i18n key including namespace, e.g. 'vergelijk:info'
 */
...
function _translateHTMLBlocks() {
    _translateHTML('info-blok', 'vergelijk:info'); // Info block: contains <strong> and <a> tags
}
...
// Load the 'vergelijk' namespace for the active language (and fallback language).
...
// to re-translate all [data-i18n] elements with 'vergelijk' now available.
i18nModule.loadNamespace('vergelijk').then(function() {
    ...
    SiteAnalytics.trackPage('vergelijk'); // Register visit in Supabase page_visits table
});
